...
* Because of the way the page is implemented we probably need to automate a real browser. WebDriver comes to mind for doing that. Be sure to allow the user to set some kind of pause between requests to not overload the server! We are good citizens. You can leverage puppeteer to be able to understand the web pages better.
...
Import that information into the database. A simple table should do with at least following columns: title of the book, URL to detail page, language, genre, flag for state (downloaded or not, skipped, scheduled for download, download failed, and so on), timestamp.

## Step 4

Allow the user to select which book to download. Mark those to scheduled for download in the database.
